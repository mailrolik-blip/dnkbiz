...
export function getAdminCourseSlugPolicy(slug: string) {
  if (getCatalogProfile(slug)) {
    return 'Slug связан с витринной карточкой курса и поэтому не редактируется из админки.';
  }

  return 'Slug задается при создании курса. Для уже созданных курсов он зафиксирован, чтобы не ломать ссылки.';
}

export function getAdminTariffSlugPolicy() {
  return 'Slug тарифа задается при создании и дальше не меняется, чтобы не ломать ссылки и историю заказов.';
}
