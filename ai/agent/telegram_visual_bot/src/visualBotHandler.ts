...
async function sendDebugJob(chatId: string, telegram: TelegramClient, stateStore: TelegramStateStore, full = false) {
  ...
  const summary = [
    ...
    `extracted_title: ${job.title_extraction?.normalized_title || job.text_layer?.text || "-"}`,
    `title_extraction_method: ${job.title_extraction?.method || "-"}`,
    `title_raw: ${job.title_extraction?.raw_command && job.title_extraction.raw_command !== job.title_extraction.normalized_title ? job.title_extraction.raw_command : "-"}`,
