...
export async function renderMonopolyPaySquare(job: VisualJob, context: RenderContext): Promise<ComposeResult> {
  ...
  const titleResult = useExistingTitle
    ...
    : await renderBrandTitleImageLayer({ text: headline, project_key: "monopoly_pay", width: placement.title_image_box.width, height: placement.title_image_box.height, tags: styleTags(job), maxLines: 2 });
