...
export function parseRevisionInstruction(instruction: string): ParsedRevisionInstruction {
  ...
  return {
    replacement_text: cleanReplacement(quoted || changeTo || textTo || write),
    remove_layer: /убери|отключи|без\s+(?:иллюстрации|фона|картинки)/iu.test(lower),
...
function detectVariant(lower: string): string | undefined {
  ...
  if (lower.includes("снизу")) return "monopoly_square_title_bottom";
