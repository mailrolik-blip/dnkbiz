...
export function reviseLayout(job: VisualJob, instruction: string): { job: VisualJob; warnings: string[] } {
  ...
  let variant = parsed.requested_variant;
  ...
  return { job: next, warnings: variants.includes(parsed.requested_variant || "") || !parsed.requested_variant ? [] : [`Requested layout is not implemented; used ${variant}.`] };
}
...
function storyVariant(job: VisualJob): string {
  if (job.project_key === "monopoly") return "monopoly_story_vertical";
